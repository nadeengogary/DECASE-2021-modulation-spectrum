...
def get_means(X_train, subgraphs):
    ...
    for G in subgraphs:
        ...
        deviations[G] = np.std([X_train[i]
                               for i in subgraphs[G]], axis=0) + 0.000001
    ...
    if mode == 'd':
        ...
        for m in machines:
            ...
            for mid in mid_dict[m]:
                ...
                AUC = roc_auc_score(y_test, y_pred_gr, max_fpr=0.9)
                pAUC = roc_auc_score(y_test, y_pred_gr, max_fpr=0.9)
        ...
        for m in machines:
            ...
            for mid in mid_dict[m]:
                ...
                X_train, X_test, eval_files = get_spectrums(
                    m, mid, reducenoise, mode)
                ...
                submission_file.to_csv(
                    '../../task2/Tiwari_IITKGP_task2_1/anomaly_score_{}_id_0{}.csv'.format(m, mid), header=False, index=False)
